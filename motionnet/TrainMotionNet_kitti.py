...
my_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[5, 10, 15, 20, 25, 30], gamma=0.5)
...
for epoch in range(epoch_num):
    ...
    for input_data, output in tqdm(dataloader_train):
        ...
        if DEBUG and count%200 == 0:
            ...
            save_temp_dir_frame_input = os.path.join(save_temp_dir_train, "epoch" + str(epoch) + "_" + str(count) +"_Input" + ".png")
            ...
            input_temp = input_data[0,-1,28:228,28:228,:].detach().cpu().numpy().astype(np.uint8) # Get 200 x 200 instead  256 x 256 grid
