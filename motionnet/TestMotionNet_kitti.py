...
test_ds = KittiDataset(directory=val_dir, device=device, num_frames=T, past_frames=past_frames, transform_pose=False, split="valid", grid_size=grid_size_fromyaml)
...
with torch.no_grad():
    ...
    for input_data, output in tqdm(dataloader_test):
        ...
        if index_num == 0:
            dir_2 = os.path.join(data_dir, "MotionNet_Prediciton")
