...
class KITTIDataset(BaseDataset):
    ...
    odometry_benchmark["2011_09_30_drive_0028_extract"] = [0, 53662]#[11271, 53567] #[11231, 53650]
    ...
    odometry_benchmark_img["2011_09_30_drive_0028_extract"] = [0, 53662] #[11271, 53567]
    ...
    def __init__(self, args):
        ...
        self.datasets_validatation_filter['2011_09_30_drive_0028_extract'] = [0, 53662] #[11271, 53567]
...
class GlobalMap:
    ...
    def __init__(self, test_scene='08', gt=False, grid_shape=200):
        ...
        # print("tlsit:", len(self.trajectory['t_list']))
        #self.trim_data()
        self.heading = self.compute_bev_heading()
        self.initialize_global_map()

        self.grid_dims = np.array([256,256])
        self.min_bound = np.array([-25.6, -25.6])
        self.max_bound = np.array([25.6, 25.6])

        # closest matched imu idx to velodyne point cloud
        closest_idx_path = './closest_idx.txt'
    ...
    def find_global_indices(self, x, y, heading):
        ...
        return x_ind, y_ind, x_pos.flatten(), y_pos.flatten()
    ...
    def generate_global_map_parameters(self, next_map, LABEL_COLORS, map_pub, arrow_pub):
        (low_bound, upper_bound) = self.frame_bounds[self.test_scene]
        n_frames = upper_bound - low_bound

        x_t = np.arange(self.global_map.shape[0]) * self.grid_size
        y_t = np.arange(self.global_map.shape[1]) * self.grid_size
        x_x, y_y = np.meshgrid(x_t, y_t, indexing='ij')
        x_x = np.expand_dims(x_x.reshape(-1), axis=1)
        y_y = np.expand_dims(y_y.reshape(-1), axis=1)

        for i in tqdm(range((n_frames))):
            labels = self.load_bev_labels(i).flatten()
            #imu_ind = i * (self.imu_freq // self.lidar_freq)
            imu_ind = int(self.closest_idx[i + 1100])
            theta = self.heading[imu_ind]
            R_t = np.eye(4,4)
            R_t[:3,:3] = self.rot[imu_ind]
            cur_pos = self.trajectory['p'][imu_ind, :]
            time_t = self.trajectory['t'][imu_ind]
            x, y = cur_pos[0], cur_pos[1]
            
            x_ind, y_ind, x_pos, y_pos = self.find_global_indices(x, y, theta)

            self.global_map[x_ind, y_ind, labels] += 1


            # Visualize
            #labels_t = np.argmax(self.global_map[..., 1:], axis=-1)+1
            labels_t = np.argmax(self.global_map, axis=-1)
            labels_t = np.expand_dims(labels_t.reshape(-1), axis=1)
            global_map_t = np.hstack([x_x, y_y, labels_t])
            map = self.publish_voxels(global_map_t, self.min_bound, self.max_bound, self.grid_dims, LABEL_COLORS, next_map)
            map_pub.publish(map)
            
            quaternion_of_R = quaternion_from_matrix(R_t)
            marker2 = Marker()
            marker2.id = 1
            marker2.ns = "arr"
            ...
            marker2.scale.y = 2.
            marker2.scale.z = 2.
            marker2.pose.position.x = x - self.min_x
            marker2.pose.position.y = y - self.min_y
            marker2.pose.position.z = 10.0
            marker2.pose.orientation.x = quaternion_of_R[0]
            marker2.pose.orientation.y = quaternion_of_R[1]
            marker2.pose.orientation.z = quaternion_of_R[2]
            marker2.pose.orientation.w = quaternion_of_R[3]
            ...
            arrow_pub.publish(marker2)

        return

    def publish_voxels(self, map_object, min_dim, max_dim, grid_dims, colors, next_map):
        ...
        semantic_labels = map_object[:,2]
        centroids = map_object[:, :2]
        ...
        for i in range(semantic_labels.shape[0]):
            pred = int(semantic_labels[i].squeeze())
            point = Point32()
            color = ColorRGBA()
...
# RGB -> BGR
# LABEL_COLORS = np.zeros_like(LABEL_COLORS_t)
# LABEL_COLORS[:,0] = LABEL_COLORS_t[:,2]
# LABEL_COLORS[:,1] = LABEL_COLORS_t[:,1]
# LABEL_COLORS[:,2] = LABEL_COLORS_t[:,0]
...
# Initialize global map object, involves computing predicted trajectory.
gm = GlobalMap(gt=True, grid_shape=200)
gm.generate_global_map_parameters(next_map, LABEL_COLORS, map_pub, arrow_pub)
